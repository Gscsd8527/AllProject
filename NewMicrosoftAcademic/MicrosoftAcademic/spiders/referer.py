...
class MicrosoftSpider(scrapy.Spider):
    ...
    def getPaperNameId():
        ...
        while 1:
            try:
                ...
                # print('data1=', data1)
                j1.add('{}tanzhenhua{}tanzhenhua{}'.format(data1['name'], data1['id'], data1["paperReferencesExpression"]))
            except:
                break
        f1.close()
        print('j1=', len(j1))
        f2 = open('referer.json', 'r', encoding='utf-8')
        j2 = set()
        while 1:
            try:
                data2 = json.loads(f2.readline().strip())
                # print('data2= ', data2)
                j2.add('{}tanzhenhua{}tanzhenhua{}'.format(data2['name'], data2['id'], data2["paperReferencesExpression"]))
            except:
                break
        f2.close()
        print('j2=', len(j2))
    ...
    def start_requests(self):
        ...
        for paper in PaperID:
            ...
            paper_dict = {
                'name': paper_list[0],
                'id': paper_list[1],
                'paperReferencesExpression': paper_list[2],
            }
    ...
    def ParseReferenceData(self, response):
        ...
        if response.status == 200:
            ...
            with open('referer.json', 'a+', encoding='utf-8') as f:
