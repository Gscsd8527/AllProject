...
class MicrosoftSpider(scrapy.Spider):
    ...
    def getPaperNameId():
        ...
        while 1:
            try:
                ...
                # print('data1=', data1)
                j1.add('{}tanzhenhua{}tanzhenhua{}'.format(data1['name'], data1['id'], data1["citedByExpression"]))
            except:
                break
        f1.close()
        print('j1=', len(j1))
        f2 = open('citedBy.json', 'r', encoding='utf-8')
        j2 = set()
        while 1:
            try:
                data2 = json.loads(f2.readline().strip())
                # print('data2= ', data2)
                j2.add('{}tanzhenhua{}tanzhenhua{}'.format(data2['name'], data2['id'], data2["citedByExpression"]))
            except:
                break
        f2.close()
        print('j2=', len(j2))
    ...
    def start_requests(self):
        ...
        for paper in PaperID:
            ...
            paper_dict = {
                'name': paper_list[0],
                'id': paper_list[1],
                'citedByExpression': paper_list[2],
            }
            paper_name = paper_dict['name']
            paper_id = paper_dict['id']
            citedByExpression = paper_dict['citedByExpression']
            if citedByExpression != '':
                reference_data = copy.deepcopy(self.data)
                reference_data['query'] = paper_name
                reference_data['queryExpression'] = citedByExpression
                reference_data['parentEntityId'] = paper_id
    ...
    def ParseReferenceData(self, response):
        ...
        if response.status == 200:
            referer_data = {
                'name': init_data['name'],
                'id': init_data['id'],
                'citedByExpression': init_data['citedByExpression']
            }
            referer_string = json.dumps(referer_data, ensure_ascii=False)
            with open('citedBy.json', 'a+', encoding='utf-8') as f:
                f.write('{}\n'.format(referer_string))

            data_json = json.loads(body)
            data_list = data_json['pr']
            Nums = data_json['t']
            print('citedBy paper_name: {}, Nums: {}'.format(paper_name, Nums))
            pages = Nums // 10
            pages_ys = Nums % 10
            if pages_ys > 0:
                pages += 1
            print('citedBy paper_name: {}, pages: {}'.format(paper_name, pages))
            ...
            for num in range(pages):
                if num == 0:
                    ...
                    next_headers['Referer'] = 'https://academic.microsoft.com/paper/{}/citedby/search?q={}&qe={}&f=&orderBy=0&skip={}&take=10'.format(init_data['id'], init_data['name'], init_data['citedByExpression'], num*10)
                    next_data = copy.deepcopy(self.data)
                    next_data['query'] = paper_name
                    next_data['skip'] = num * 10
                    next_data['queryExpression'] = init_data['citedByExpression']
                    next_data['parentEntityId'] = init_data['id']
                    meta = {
                        'init_data': init_data,
                        'type': type,
                        'num': num,
    ...
    def PargeTurning(self, response):
        ...
        print('citedBy 这是 {} 第 {} 页'.format(init_data['name'], num))
        data_json = json.loads(body)
        data_list = data_json['pr']
        papers_list = self.ParsePaperData(data_list, init_data, type, num)
        for sig_paper in papers_list:
