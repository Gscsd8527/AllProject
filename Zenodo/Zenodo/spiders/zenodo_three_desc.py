...
class ZenodoSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for key in key_list:
            init_url = 'https://zenodo.org/api/records/?page=1&size=20&q={}&access_right=open&access_right=embargoed&type=dataset&sort=-bestmatch'.format(key)
            # headers['Referer'] = 'https://zenodo.org/search?page=1&size=20&q={}&access_right=open&access_right=embargoed'.format(key)
            headers['Referer'] = 'https://zenodo.org/search?page=1&size=20&q={}&access_right=open&access_right=embargoed&type=dataset'.format(key)
            ...
            if response.status_code == 200:
                ...
                for page in range(1, pages + 1):
                    url = 'https://zenodo.org/api/records/?page={}&size=20&q={}&access_right=open&access_right=embargoed&type=dataset&sort=-bestmatch'.format(page, key)
                    if page == 1:
                        Referer = 'https://zenodo.org/search?page=1&size=20&q={}&access_right=open&access_right=embargoed&type=dataset'.format(key)
                    else:
                        Referer = 'https://zenodo.org/search?page={}&size=20&q={}&access_right=open&access_right=embargoed&type=dataset&sort=-bestmatch'.format(page-1, key)
