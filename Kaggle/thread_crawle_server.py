...
def start_thread(page_urls):
    ...
    xs = x + 2
    for i in range(xs):
        print('循环第  {}  次， 共有  {}  次'.format(i, xs))
        print('****************')
        if i == x + 1:
            urls = page_urls[i*THREAD_NUM:]
        else:
            urls = page_urls[i*THREAD_NUM: (i+1)*THREAD_NUM]
        threads = [threading.Thread(target=parsePages, args=(ul,)) for ul in urls]
